...
| **[danielblnc / DLSS-NR-on-AMD](https://github.com/danielblnc/DLSS-NR-on-AMD)** | AMD 神经渲染运行时本体（0.3.0 / 0.3.1 / 0.4.0 / 0.4.1） | **不改动其核心**，按规范接口调用；并针对 0.3.1 / 0.4.x 的 1 像素 Draw 等待补齐状态保护，确保在 DLSS/XeSS 游戏上安全运行 |
...
- **`danielblnc` 专属**：
  ...
  - **仅 0.4.x**：`Style`（默认 / 自然 / 电影感）、`Tone curve`（Reinhard / ACES）、`Black lift`（0～0.25）、`Exposure`（游戏提供 / 自动曝光）以及 `Tone intensity`（0～2）。
