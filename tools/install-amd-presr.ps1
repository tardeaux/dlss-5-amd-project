...
.SYNOPSIS
  ...
  Copies danielblnc's 0.4.1 / 0.4.0 / 0.3.3 / 0.3.2 / 0.3.1 / 0.3.0 runtime (version.dll) to dlssnr_amd_pass1-3.dll,
...
.DESCRIPTION
  ...
  Put these in the SAME folder as Setup.ps1 (the package root):
    ...
    version.dll                 danielblnc AMD NR 0.4.1 / 0.4.0 / 0.3.3 / 0.3.2 / 0.3.1 / 0.3.0 (copied to pass1-3)
    nvngx_dlssnr.dll            optional, to generate weights with danielblnc setup
    dlssnr_on_amd_setup.exe     optional, danielblnc 0.4.1 / 0.4.0 / 0.3.3 / 0.3.2 / 0.3.1 / 0.3.0 setup
...
# 0.3.0 = kAmd03; 0.3.1 = kAmd031; 0.3.2 = kAmd032; 0.3.3 = kAmd033; 0.4.0 = kAmd040; 0.4.1 = kAmd041.
...
$expectedAuthor = @($expectedA030, $expectedA031, $expectedA032, $expectedA033, $expectedA040, $expectedA041)
...
$srcA is not a supported DLSS-NR-on-AMD runtime ($what).
  file:     $srcA
  got:      $hashA
  expected: $expectedA030 (0.3.0) .. $expectedA041 (0.4.1)
Download 0.3.0–0.4.1 from https://github.com/danielblnc/DLSS-NR-on-AMD/releases
"@
    }
